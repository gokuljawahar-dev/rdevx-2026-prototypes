(() => {
  ...
  const PRODUCT = {"id": "gateway", "label": "Gateway", "signature": "candidate", "signatureSelector": ".gateway-stage-rail li", "responses": ["Your confirmed 4:00 PM slot is preserved. The only unresolved gate is camera readiness, so recording and interview start remain blocked until retest passes.", "Consent for recording and AI-supported evaluation is purpose-specific. No choice is preselected, and silence is never recorded as consent.", "The scorecard can expose evidence and limitations, but an authorized Hiring Manager records the screening disposition."], "stateNoun": "screening instrument"};
