(() => {
  ...
  const PRODUCT = {"id": "horizon", "label": "Horizon", "signature": "cockpit", "signatureSelector": ".attention-row", "responses": ["ENG-042 is ranked first because the capacity variance is material, the release commitment is near, and the supporting evidence is current.", "The smallest permissible next operation is to prepare the capacity or commitment response. Cross-lever effects remain simulated until named authority acts.", "Release evidence is Unknown, so Horizon will not prepare a definitive Go recommendation. Refresh, wait or request the minimum missing fact."], "stateNoun": "governance instrument"};
